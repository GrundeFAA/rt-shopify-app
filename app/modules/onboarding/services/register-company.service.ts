...
function getDeliveryAddress(payload: RegisterCompanyPayload) {
  ...
  const data = await executeAdminGraphql({
    ...
    variables: {
      input: {
        ...
        ...(payload.user.phone ? { phone: payload.user.phone } : {}),
      },
    },
  });
...
async function getCompanyById(
  ...
  const data = await executeAdminGraphql({
    ...
    operationName: "CompanyForOnboarding",
...
async function ensureCompanyContact(
  ...
  const data = await executeAdminGraphql({
    ...
    operationName: "RegisterCompanyContact",
...
async function ensureMainCompanyContact(
  ...
  await executeAdminGraphql({
    ...
    operationName: "RegisterCompanyMainContact",
...
async function ensureLocationAdminRole(
  ...
  await executeAdminGraphql({
    ...
    operationName: "RegisterCompanyContactRoles",
...
async function syncCompanyMetafields(
  ...
  await executeAdminGraphql({
    ...
    operationName: "RegisterCompanyMetafields",
...
export async function registerCompany(
  context: AdminServiceContext,
  rawPayload: unknown,
): Promise<RegisterCompanySuccess> {
  const payload = RegisterCompanyPayloadSchema.parse(rawPayload);
  ...
  const existingCustomer = await findCustomerByEmail(context, payload.user.email);

  if (existingCustomer) {
    throw new AppError(
      "CUSTOMER_ALREADY_EXISTS",
      "An account with this email already exists. Sign in or contact support.",
      409,
      false,
      {
